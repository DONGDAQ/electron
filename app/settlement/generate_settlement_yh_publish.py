...
def read_feishu_data(year, month):
    """从飞书读取指定月份的交付数据"""
    client = FeishuClient('S5yHmP')
    data = client.read_sheet('S5yHmP', 'A1:O')
...
def generate_settlement_excel(records, year, month, output_dir):
    """生成结算单Excel文件"""
    template = TEMPLATE_DIR / '【异环结算单】-NTE【发行】模板.xlsx'
    filename = f'【异环结算单】-NTE【发行】{year}年{month}月.xlsx'
    ...
    if output_path.exists():
        try:
            ...
            filename = f'【异环发行结算单】-NTE【发行】{year}年{month}月_{ts}.xlsx'
            output_path = output_dir / filename
    ...
    for i, rec in enumerate(records):
        ...
        # D列: Task No (Feishu E) - 异环发行没有这一列，填充-
...
def generate_acceptance_docx(records, year, month, output_dir):
    """生成验收单Word文件"""
    template = TEMPLATE_DIR / '【异环】验收单-NTE【发行】模板.docx'
    filename = f'【异环】验收单-NTE【发行】{year}年{month}月.docx'
    output_path = output_dir / filename
    ...
    if output_path.exists():
        try:
            ...
            filename = f'【异环发行】验收单-TOF【发行】{year}年{month}月_{ts}.docx'
            output_path = output_dir / filename
    ...
    number_str = today.strftime('%Y%m%d') + '03'
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description='异环发行月度结算单生成')
    ...
    output_dir = Path(args.output) if args.output else get_settlement_dir() / f"{year}年{month}月" / "完美世界" / "异环发行"
