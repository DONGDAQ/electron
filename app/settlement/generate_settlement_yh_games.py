...
def read_feishu_data(year, month):
    """从飞书读取指定月份的交付数据"""
    client = FeishuClient('bfba7c')
    data = client.read_sheet('bfba7c', 'A1:O')
...
def generate_settlement_excel(records, year, month, output_dir):
    """生成结算单Excel文件"""
    template = TEMPLATE_DIR / '【异环结算单】-NTE【游戏内】年月模板.xlsx'
    filename = f'【异环结算单】-NTE【游戏内】{year}年{month}月.xlsx'
    ...
    if output_path.exists():
        try:
            ...
            filename = f'【异环游戏内结算单】-NTE【游戏内】{year}年{month}月_{ts}.xlsx'
            output_path = output_dir / filename
...
def generate_acceptance_docx(records, year, month, output_dir):
    """生成验收单Word文件"""
    template = TEMPLATE_DIR / '【异环】验收单-NTE【游戏内】年月模板.docx'
    filename = f'【异环】验收单-NTE【游戏内】{year}年{month}月.docx'
    output_path = output_dir / filename
    ...
    if output_path.exists():
        try:
            ...
            filename = f'【异环游戏内】验收单-TOF【游戏内】{year}年{month}月_{ts}.docx'
            output_path = output_dir / filename
    ...
    number_str = today.strftime('%Y%m%d') + '02'
...
def main():
    ...
    output_dir = Path(args.output) if args.output else get_settlement_dir() / f"{year}年{month}月" / "完美世界" / "异环游戏内"
